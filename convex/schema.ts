...
export default defineSchema({
  ...
  titles:defineTable({ title:v.string(), author:v.string(), isbn:v.string(), ...stock, notes:v.optional(v.string()), synopsis:v.optional(v.string()), coverUrl:v.optional(v.string()), purchaseInfo:v.optional(v.string()), supplierIds:v.optional(v.array(v.id("suppliers"))), enrichmentSource:v.optional(v.object({ source:v.literal("openLibrary"), fetchedAt:v.number() })), reorderNeeded:v.boolean() }).index("by_isbn",["isbn"]),
  people:defineTable({
    name:v.string(),
  ...
  orders:defineTable({ supplierId:v.id("suppliers"), status:v.union(v.literal("needed"),v.literal("ordered"),v.literal("received")), expectedAt:v.optional(v.number()), orderedAt:v.optional(v.number()) }),
  orderLines:defineTable({ orderId:v.id("orders"), titleId:v.id("titles"), orderedQuantity:v.number(), receivedQuantity:v.number() }).index("by_order",["orderId"]).index("by_title",["titleId"]),
